...
setMethod("show", "APPconf", function(object){
  ...
  cat("First 5 markers for current data: ", head(markers(object), n=5L), "\n")
...
setMethod("[", "APPconf", function(x, i, j, ..., drop=TRUE){
  as(x, "list")[i]
})
...
#' @examples
#' as.list(appconf)
setMethod("as.list", "APPconf",
          function(x, ...){
            ii <- slotNames(x)
            names(ii) <- ii
            lapply(ii, function(i) x[[i]])
          })
setAs(from="APPconf", to="list",
      function(from){
        ii <- slotNames(from)
        names(ii) <- ii
        lapply(ii, function(i) from[[i]])
      })
if(length(getGeneric("as.character"))==0){
  setGeneric("as.character", function(x, ...) standardGeneric("as.character"))
}
#' @rdname APPconf-methods
#' @export
#' @importFrom methods slotNames setAs
...
setMethod("as.character", "APPconf",
          function(x, ...){
            ii <- slotNames(x)
            c(markers(x), unlist(x[ii!='markers']))
          })
setAs(from="APPconf", to="character",
      function(from){
        ii <- slotNames(from)
