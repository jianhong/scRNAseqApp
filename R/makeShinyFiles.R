...
makeShinyFiles <- function(
    obj, scConf, assayName,
    gexSlot = c("data", "scale.data", "counts"),
    appDir = "data",
    defaultGene1 = NA, defaultGene2 = NA,
  ...
  # Make XXXgexpr.h5
  if(!dir.exists(appDir)){dir.create(appDir)}
  ...
  if(all(default.dimred %in% sc1conf[sc1conf$dimred == TRUE]$ID)){
    ...
    if(length(grep(guess, sc1conf[sc1conf$dimred == TRUE]$UI,
                   ignore.case=TRUE)) >= 2){
  ...
  sc1def <- list()
  sc1def$meta1 <- sc1conf[sc1conf$default == 1]$UI   # Use display name
  sc1def$meta2 <- sc1conf[sc1conf$default == 2]$UI   # Use display name
  ...
  if(tmp == 2){
    ...
    sc1def$grp1 <- sc1conf[sc1conf$default != 0 & sc1conf$grp == TRUE]$UI
    if(nrow(sc1conf[sc1conf$default == 0 & sc1conf$grp == TRUE]) == 0){
      sc1def$grp2 <- sc1def$grp1
    } else {
      sc1def$grp2 <- sc1conf[sc1conf$default == 0 & sc1conf$grp == TRUE]$UI[1]
    }
  } else {
    sc1def$grp1 <- sc1conf[sc1conf$default == 0 & sc1conf$grp == TRUE]$UI[1]
    if(nrow(sc1conf[sc1conf$default == 0 & sc1conf$grp == TRUE]) < 2){
      sc1def$grp2 <- sc1def$grp1
    } else {
      sc1def$grp2 <- sc1conf[sc1conf$default == 0 & sc1conf$grp == TRUE]$UI[2]
    }
  }
