plotMonocleUI <- function(id){
  ...
  tabPanel(
    ...
    "In this tab, users can visualise the cells along with their trajectories.",
    br(),br(),
    fluidRow(
      column(
        3, style="border-right: 2px solid black",
        selectInput(ns("group"), "select the data group",
                    choices = c(), selected = NULL),
        selectInput(ns("root"), "select the root point",
                    choices = c(), selected = NULL),
        checkboxInput(ns("show_trajectory_graph"),
                      "show trajectory graph",
                      value = TRUE),
        checkboxInput(ns("label_principal_points"),
                      "label principal points",
                      value = TRUE),
        checkboxInput(ns("label_leaves"),
                      "label leaves",
                      value = TRUE),
        checkboxInput(ns("label_roots"),
                      "label roots",
                      value = TRUE),
...
plotMonocleServer <- function(id, dataSource, optCrt){
  if(checkMisc("monocle", dataSource()$dataset)){
    ...
    if(is.null(reduction_method)) reduction_method <- "UMAP"
    ...
    if(length(monocle)){
      ...
      plot1 <- reactive({
        scDRmonocle(
          ...
          graph_label_size = .globals$sList[input$plotfsz]/6,
