  ...
  moduleServer(id, function(input, output, session){
    ...
    observeEvent(input$monocle, {
      ...
      adminProcess({
        ...
        reduction_method <- seuReductionMethod(seu)
        miscData <- addMonocle(cds, reduction_method, seu[[]], global$config)
    ...
    observeEvent(input$cellchat, {
      ...
      adminProcess({
        seu <- getSeuObj()
        grp_ids <- filterGrpIDs(getGrpIDs(global$config), seu[[]])
        ...
        misc_cellchat <- lapply(grp_ids, function(grp){
          addCellChat(seu, seu[[]], grp, input$species)
        })
        message("Assign cellchat to miscellaneous data")
